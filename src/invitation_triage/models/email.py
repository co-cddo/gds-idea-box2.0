...
class SafeEmail(BaseModel):
    ...
    received_date: datetime
    has_attachments: bool = False
    pii_extracted: dict[str, list[str]]  # {"emails": [...], "phone_numbers": [...], "names": [...]}
    
    @classmethod
    def from_raw_email(cls, raw_email: RawEmail) -> "SafeEmail":
        """
        Create SafeEmail by extracting and redacting PII from RawEmail.
        
        PII types detected:
        - Email addresses
        - Phone numbers
        - Names (using NER - future)
        - Addresses (future)
        """
        # Extract PII
