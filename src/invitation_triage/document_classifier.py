...
from invitation_triage.models import SafeDocument, DocumentClassification
...
async def classify_document(safe_document: SafeDocument) -> DocumentClassification:
    """
    Classify document type from safe document.

    Simple agent with no tools - pure classification based on text and metadata.

    Args:
        safe_document: Document with PII-redacted text and metadata
...
"""
    ...
    logger.info(
        ...
        extra={
            "document_id": safe_document.document_id,
            "text_length": len(safe_document.safe_text),
            "source_type": safe_document.source_type,
        },
    )

    try:
        result = await agent.run("Classify this document type.", deps=safe_document)
        ...
        logger.error(
            ...
            extra={"document_id": safe_document.document_id},
            exc_info=True,
        )
        raise ClassificationError(
            f"LLM failed to classify document: {str(e)}",
            text_preview=safe_document.safe_text[:200],
            cause=e,
        ) from e
