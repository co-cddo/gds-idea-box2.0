...
def test_route_calls_handler_with_items(config):
    """Handler should be called once per item returned by get_items."""
    handler = AsyncMock()
    ...
    assert handler.call_args_list[0].args[0]["id"] == "1"
    assert handler.call_args_list[1].args[0]["id"] == "2"
...
def test_route_filters_self_writes(config):
    ...
    assert handler.call_args.args[0]["id"] == "human-wrote"
...
def test_route_notification_dedup(config):
    """Sending the same notification twice should only invoke the handler once."""
    handler = AsyncMock()
    items = [_make_item()]
    route = WebhookRoute(path="/test", get_items=lambda: items, handler=handler, filter_self=False)
...
def test_route_item_level_dedup(config):
    ...
    # Two notifications with different subscription IDs (bypasses notification dedup)
    n1 = {**VALID_NOTIFICATION, "subscriptionId": "sub-1"}
    n2 = {**VALID_NOTIFICATION, "subscriptionId": "sub-2"}
...
def test_multiple_routes_dispatch_to_correct_handler(config):
    ...
    handler_a = AsyncMock()
    handler_b = AsyncMock()
    items_a = [_make_item(item_id="a1")]
    items_b = [_make_item(item_id="b1")]
